        ...
        <div className='max-lg:hidden'>
          ...
          <ul className='space-y-4 text-gray-600'>
            {JobLocations.map((location, index) => (
              <li key={index} className='flex gap-3 items-center'>
                <input type="checkbox" onChange={() => handleLocationsChange(location)} checked={selectedLocations.includes(location)} className='scale-125' />
                {location}
              </li>
            ))}
          </ul>
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        {jobs.length > 0 && (
          <div className='flex items-center justify-center space-x-2 mt-10'>
            <a href='#job-list'>
              <img onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} src={assets.left_arrow_icon} alt='Previous' />
            </a>
            ...
            <a href='#job-list'>
              <img onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(jobs.length / 6)))} src={assets.right_arrow_icon} alt='Next' />
            </a>
          </div>
        )}
      </section>
